  ...
  return (
    <div className="bg-white border-b px-3 md:px-4 py-2 md:py-3 flex justify-between items-center">
      ...
      <div className="flex space-x-1 md:space-x-2">
        <Button onClick={handleImport} variant="secondary" className="text-xs md:text-sm px-2 md:px-4 py-1 md:py-2 md:w-[100px] md:flex">
          <Download className="w-3 h-3 md:w-4 md:h-4 md:mr-2" />
          <span className="hidden sm:inline">Import</span>
        </Button>
        <Button onClick={handleExport} disabled={sections.length === 0} className="text-xs md:text-sm px-2 md:px-4 py-1 md:py-2 md:w-[100px] md:flex">
          <Upload className="w-3 h-3 md:w-4 md:h-4 md:mr-2" />
          <span className="hidden sm:inline">Export</span>
        </Button>
        <Button onClick={handleClear} variant="danger" disabled={sections.length === 0} className="text-xs md:text-sm px-2 md:px-4 py-1 md:py-2 md:w-[100px] md:flex">
          <Trash2 className="w-3 h-3 md:w-4 md:h-4 sm:mr-1 md:mr-2" />
